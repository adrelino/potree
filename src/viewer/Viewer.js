...
class PotreeViewer extends THREE.EventDispatcher {
	constructor (domElement, args) {
		...
		this.fov = 60;
		this.clipMode = ClipMode.HIGHLIGHT_INSIDE;
		this.isFlipYZ = false;
		...
		{ // set defaults
			...
			this.setClipMode(ClipMode.HIGHLIGHT_INSIDE);
	...
	loadSettingsFromURL () {
		...
		if (getParameterByName('clipMode')) {
			let clipMode = getParameterByName('clipMode');
			if (clipMode === 'HIGHLIGHT_INSIDE') {
				this.setClipMode(ClipMode.HIGHLIGHT_INSIDE);
			} else if (clipMode === 'CLIP_OUTSIDE') {
				this.setClipMode(ClipMode.CLIP_OUTSIDE);
			} else if (clipMode === 'DISABLED') {
				this.setClipMode(ClipMode.DISABLED);
			}
		}
